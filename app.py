...
def add_student():
    try:
        ...
        c.execute('''INSERT INTO students (
            ...
            data.get('score_初三上期末', ''), data.get('score_一模', ''), data.get('score_二模', ''),
...
def get_students():
    ...
    for s in students:
        result.append({
            ...
            'score_二模': s['score_二模'], 'test_paper': s['test_paper'],
...
def update_student(student_id):
    ...
    try:
        c.execute('''UPDATE students SET
            ...
            rank_初三上期中=?, rank_初三上期末=?, score_初三上期末=?,
            score_一模=?, score_二模=?, test_paper=?, test_location=?,
